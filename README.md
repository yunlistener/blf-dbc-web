...
| **M2 前端核心可视化**(CANoe 风格曲线窗口) | ✅ 完成(信号曲线/光标读数/框选+滚轮缩放) |
| **M2 剩余**(Trace 报文表格、多信号叠加、ID 统计图、CSV 导出) | ⏳ 规划中 |
| **M3 树莓派部署**(systemd 服务、局域网访问、性能验证) | ⏳ 规划中 |
...
| 5 | 可视化 | 信号时序曲线(时间轴缩放/光标读数/多信号叠加)、ID 分布图 | P1 | ✅ 单信号曲线;叠加/统计图 ⏳ |
| 6 | 数据导出 | 解码结果导出 CSV / JSON | P1 | ⏳ |
...
- 报文/信号树:按报文展开信号,点击加载曲线
- **信号时序曲线**(uPlot):十字光标实时读数(时间/信号/数值/单位)
